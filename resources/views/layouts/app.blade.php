...
<div class="flex h-screen overflow-hidden" x-data="{ sidebarOpen: false }">
    ...
    <aside
        ...
        class="fixed inset-y-0 left-0 z-30 flex flex-col w-64 bg-neutral-800 text-gray-100 flex-shrink-0 overflow-y-auto transform transition-transform duration-200 ease-in-out lg:relative lg:translate-x-0 lg:z-auto">
        ...
        {{-- Logo --}}
        <div class="flex flex-col px-5 py-5 border-b border-neutral-700 gap-1">
        ...
        <nav class="flex-1 px-3 py-4 space-y-1">
            <a href="{{ route('dashboard') }}"
               class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition
                      {{ request()->routeIs('dashboard') ? 'bg-neutral-700 text-white' : 'text-gray-300 hover:bg-neutral-700 hover:text-white' }}">
            ...
            <div class="pt-3">
                ...
                <a href="{{ route('projects.show', $sidebarProject) }}"
                   class="flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm transition
                          {{ request()->route('project')?->id === $sidebarProject->id ? 'bg-neutral-700 text-white' : 'text-gray-300 hover:bg-neutral-700 hover:text-white' }}">
        ...
        <div class="border-t border-neutral-700 px-3 py-3 space-y-1" x-data="{ userOpen: false }">
            {{-- Notification bell --}}
            <a href="{{ route('notifications.index') }}"
               class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-gray-300 hover:bg-neutral-700 hover:text-white transition">
                <div class="relative">
            ...
            <div class="relative">
                <button @click="userOpen = !userOpen"
                        class="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-gray-300 hover:bg-neutral-700 hover:text-white transition">
                ...
                <div x-show="userOpen" @click.outside="userOpen = false" x-transition
                     class="absolute bottom-full left-0 mb-1 w-full bg-neutral-700 border border-neutral-600 rounded-lg shadow-lg overflow-hidden z-50">
                    <a href="{{ route('profile.edit') }}" class="block px-4 py-2 text-sm text-gray-300 hover:bg-neutral-600">Profile</a>
                    @if(auth()->user()->isAdmin())
                    <a href="{{ route('admin.users.index') }}" class="block px-4 py-2 text-sm text-gray-300 hover:bg-neutral-600">Admin: Users</a>
                    <a href="{{ route('admin.projects.archived') }}" class="block px-4 py-2 text-sm text-gray-300 hover:bg-neutral-600">Admin: Archived Projects</a>
                    @endif
                    ...
                    <form method="POST" action="{{ route('impersonate.stop') }}">
                        @csrf
                        <button type="submit" class="w-full text-left px-4 py-2 text-sm text-yellow-400 hover:bg-neutral-600">
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="w-full text-left px-4 py-2 text-sm text-red-400 hover:bg-neutral-600">Logout</button>
                    </form>
                </div>
            </div>
        </div>
    ...
    <div class="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        {{-- ── Mobile top bar ──────────────────────────────────────── --}}
        <header class="flex items-center gap-3 px-4 py-3 bg-neutral-800 text-white lg:hidden flex-shrink-0">
