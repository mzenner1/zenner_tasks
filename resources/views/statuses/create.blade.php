<x-app-layout title="New Status">
<div class="max-w-lg space-y-6">
    <div class="flex items-center gap-2 text-sm text-gray-500">
        <a href="{{ route('projects.statuses.index', $project) }}" class="hover:text-indigo-600">Statuses</a>
        <span>/</span>
        <span class="text-gray-800 font-medium">New Status</span>
    </div>
    <h1 class="text-2xl font-bold text-gray-900">New Status</h1>
    ...
    <form method="POST" action="{{ route('projects.statuses.store', $project) }}"
        ...
        <div class="flex items-center gap-6">
            ...
            <label class="flex items-center gap-2 text-sm cursor-pointer">
                <input type="checkbox" name="is_closed" value="1" {{ old('is_closed') ? 'checked':'' }}
